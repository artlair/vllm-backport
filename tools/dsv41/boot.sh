...
run_podman() {
  ...
  local podman_args=(
    ...
    ${VLLM_PP_LAYER_PARTITION:+-e VLLM_PP_LAYER_PARTITION=$VLLM_PP_LAYER_PARTITION}
    ${VLLM_DSV41_PP_KV_RELAY:+-e VLLM_DSV41_PP_KV_RELAY=$VLLM_DSV41_PP_KV_RELAY}
    ${VLLM_PP_CACHED_METADATA:+-e VLLM_PP_CACHED_METADATA=$VLLM_PP_CACHED_METADATA}
    ${NCCL_DEBUG:+-e NCCL_DEBUG=$NCCL_DEBUG}
    ${VLLM_INDEXER_QUERY_SHARD:+-e VLLM_INDEXER_QUERY_SHARD=$VLLM_INDEXER_QUERY_SHARD}
