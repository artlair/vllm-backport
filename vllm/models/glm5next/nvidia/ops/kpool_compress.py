...
def _fwht_quant_kernel(
    ...
    # Encode to e4m3fn bytes explicitly: Triton refuses the implicit
    # fp32 -> fp8e4nv store conversion below SM89 (see fp8_sm80.py); the
    # encoder keeps the hardware convert on SM89+.
    tl.store(
...
def _kpool_softmax_rotate_write_cache_kernel(
    ...
    # Explicit e4m3fn byte encode (implicit fp32 -> fp8 stores fail < SM89).
...
def kpool_compress_and_write_cache(
    ...
    # The kernel encodes e4m3fn bytes itself (SM<89 has no fp8 store convert),
    # so the fp8 destinations are handed over as uint8 views.
...
def kpool_decode_update_and_maybe_write_cache_batched(
    ...
    page_size = kv_cache.shape[1]
    buf = kv_cache
    # uint8 destination: the kernel encodes e4m3fn bytes itself (SM<89 has
    # no fp8 store convert).
    buf_fp8 = buf
    buf_fp32 = buf.view(torch.float32)
