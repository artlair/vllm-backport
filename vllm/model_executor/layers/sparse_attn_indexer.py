...
def _two_level_select(
    ...
    """Two-level selection (v4.1): the candidate source publishes its top
    blocks (``write``); later indexers mask their scores to them. Runs before
    the row top-k. Rows are independent, so callers may pass any row slice as
    long as ``candidates`` is sliced the same way (our row-chunked SM80
    prefill path and the TP query-sharded decode path rely on this)."""
    if write:
        _select_candidate_blocks(
            logits, row_ks, row_ke, candidates.shape[1], block_size, candidates
        )
    else:
        _apply_candidate_mask(logits, row_ks, row_ke, candidates, block_size)
...
def sparse_attn_indexer(
            ...
            if chunk.local_total_seq_lens == 0:
                ...
                if current_platform.is_xpu():
                    ...
                    for r0 in range(0, num_rows, row_step):
                        ...
                        if candidate_blocks is not None:
                            # Rows are independent, so the v4.1 two-level
                            # selection runs per row block on the same slices.
                            _two_level_select(
                                logits,
                                cu_seqlen_ks[r0:r1],
                                cu_seqlen_ke[r0:r1],
    ...
    if has_decode:
        ...
        if candidate_blocks is not None:
            ...
            # otherwise. The rows here are this rank's query-sharded slice
            # [row_lo, row_hi), so slice the candidate buffer the same way.
            ...
            _two_level_select(
                ...
                row_ks,
                vis,
                candidate_blocks[row_lo : row_lo + num_rows],
                candidate_block_size,
                candidate_write,
            )
