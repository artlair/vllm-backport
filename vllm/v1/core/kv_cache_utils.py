...
def _annotate_eagle_groups_glm5_next(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
) -> None:
    """Flag the GLM-5.3-Flash draft group when the spec rule found nothing.

    Glm5NextMTP's attention is an ordinary sparse-MLA layer (it never sets
    ``non_causal_multi_token_decode``), and ``_get_kv_cache_groups_glm5_next``
    aliases every MLA layer, the drafter's included, into group 0. Leaving all
    ...
    for group in kv_cache_groups:
        if not any(
            isinstance(spec, (MambaSpec, KpoolTailSpec))
...
def get_kv_cache_groups(
    ...
    if is_kv_cache_spec_uniform(kv_cache_spec):
        ...
        _annotate_eagle_groups(vllm_config, kv_cache_spec, glm5_groups)
        _annotate_eagle_groups_glm5_next(vllm_config, glm5_groups)
        return glm5_groups
