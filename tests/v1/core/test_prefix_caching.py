...
def _prefill_with_mamba_align_split(manager, request, block_size, max_tokens):
    """Prefill ``request`` in the chunks the real align-mode splitter picks."""
    ...
    new_computed = num_computed
    ends = []
    while request.num_computed_tokens + new_computed < request.num_tokens:
        start = request.num_computed_tokens + new_computed
        num_new = Scheduler._mamba_block_aligned_split(
            stub,
            request,
            min(request.num_tokens - start, max_tokens),
            new_computed,
        )
        assert num_new > 0
        blocks = manager.allocate_slots(
            ...
            num_new,
            new_computed,
            computed_blocks if new_computed else None,
...
def test_hybrid_mamba_retention_mtp_eagle_on_attention_group_only(prompt_len):
    ...
    The drafter's MLA layer is aliased into the attention group, so that is the
    one group the grouping flags (``_annotate_eagle_groups_glm5_next``); the
    Mamba (KDA) groups are not eagle groups. The attention group's EAGLE drop
    still lands the whole hit one block down, so the Mamba groups must retain
    the state there although they never drop themselves. Retaining only the
    ``num_prompt - 1`` replay boundary made the first identical resend and the
    first extension of every prompt a full re-prefill.
    """
    block_size = 32
    num_spec = 3
    ...
    kv_cache_config = KVCacheConfig(
        ...
        kv_cache_groups=[
            ...
            KVCacheGroupSpec(["kda.0"], mamba_spec),
            KVCacheGroupSpec(["kda.1"], mamba_spec),
        ],
    )
    manager = make_kv_cache_manager(
        kv_cache_config=kv_cache_config,
    ...
    fresh = make_request("fresh", token_ids, block_size, sha256)
    _prefill_with_mamba_align_split(manager, fresh, block_size, max_tokens=4096)
    manager.free(fresh)

    # Attention matches up to the last full block below num_prompt - 1 and the
    # EAGLE drop takes one block off it.
    resend_hit = (prompt_len - 1) // block_size * block_size - block_size
    resend = make_request("resend", token_ids, block_size, sha256)
    _, num_computed_tokens, _ = manager.get_computed_blocks(resend)
    assert num_computed_tokens == resend_hit

    extension_hit = prompt_len // block_size * block_size - block_size
    extension = make_request("extension", token_ids + [-1] * 40, block_size, sha256)
    _, num_computed_tokens, _ = manager.get_computed_blocks(extension)
    assert num_computed_tokens == extension_hit
