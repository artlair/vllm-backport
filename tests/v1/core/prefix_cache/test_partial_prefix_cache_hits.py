...
@pytest.mark.parametrize("eagle_on_attention_only", [False, True])
def test_mamba_partial_tail_follows_attention_eagle_drop(eagle_on_attention_only):
    """The partial tail shift must not depend on the Mamba group's eagle bit.

    With the draft group identified (GLM-5.3-Flash flags only its aliased MLA
    group) the Mamba group is not an eagle group, yet the attention group's
    drop still lands the hit one hash unit below the prompt's last boundary.
    """
    block_size, hash_block_size = 1024, 64
    config = _make_hybrid_kv_cache_config(block_size, 64, ["full", "mamba_align"])
    if eagle_on_attention_only:
        groups = list(config.kv_cache_groups)
        groups[0] = replace(groups[0], is_eagle_group=True)
    ...
    assert stub.mamba_partial_cache_hit

    prompt = list(range(1600))
    req = make_request("A", prompt, hash_block_size, sha256)
    while req.num_computed_tokens < req.num_tokens:
        num_new = Scheduler._mamba_block_aligned_split(
            stub, req, req.num_tokens - req.num_computed_tokens
        )
        assert manager.allocate_slots(req, num_new) is not None
        req.num_computed_tokens += num_new
        manager.new_step_starts()
    manager.free(req)

    sibling = make_request("B", prompt + [-1] * 500, hash_block_size, sha256)
    _, num_computed, _ = manager.get_computed_blocks(sibling)
    assert num_computed == len(prompt) - hash_block_size
